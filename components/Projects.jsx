    ...
    <section id="projects" className="py-20 w-full scroll-mt-20">
      ...
      >
        <h2 className="text-4xl md:text-6xl font-black mb-12 uppercase tracking-tighter inline-block relative">
          Featured Works
          <div className="absolute bottom-0 left-0 w-full h-4 bg-primary -z-10 -rotate-1 hidden md:block"></div>
        </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
          {projects.map((project, index) => (
            <motion.div 
              ...
              className="group bg-background border-[3px] border-[var(--border-color)] p-6 md:p-8 flex flex-col justify-between shadow-[8px_8px_0px_var(--border-color)] hover:shadow-[12px_12px_0px_var(--primary)] hover:-translate-y-2 hover:-translate-x-2 transition-all duration-300 h-full"
            >
              <div>
              ...
              <div>
                <div className="flex flex-wrap gap-2 mb-8">
                  {project.tech.map((tech) => (
                    <span key={tech} className="px-3 py-1 bg-[var(--border-color)] text-background text-sm font-bold border-2 border-transparent group-hover:bg-background group-hover:text-foreground group-hover:border-[var(--border-color)] transition-colors">
                      {tech}
                    </span>
                  ))}
                </div>
